...
@AllArgsConstructor
public class GetParameterMinimunAsignationUseCase extends Usecase<RequestParams, Mono<ParametersResponseWithColumnNames>> {
    ...
    public Mono<ParametersResponseWithColumnNames> apply(RequestParams requestParams) {
        return productRepository.getByPageAndSize(requestParams.getPage(), requestParams.getSize())
                .flatMap(product -> parameterMinimumRepository.getByProductCode(product.getCodigo())
                        .collectList()
                        .map(parameterSizePerformanceDTOS -> ProductMinimum.builder()
                                .productId(product.getCodigo())
                                .description(product.getDescripcion())
                                .species(product.getEspecie())
                                .minimums(parameterSizePerformanceDTOS.stream()
